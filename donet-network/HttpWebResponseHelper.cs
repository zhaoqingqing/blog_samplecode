...
namespace LogicModule
{
	...
	public class HttpWebResponseHelper
	{
		...
		/// <returns></returns>
		public static HttpWebResponse CreateGetHttpResponse(string url, int? timeout, string userAgent, CookieCollection cookies, Action<string> errorCallback)
		{
			if (string.IsNullOrEmpty(url))
			{
				if (errorCallback != null) errorCallback("please check ,request url is null");
				return null;
			}
			...
			if (request == null)
			{
				if (errorCallback != null) errorCallback(string.Format("404 ! request is null ,url={0}", url));
				return null;
			}
			request.Method = "GET";
